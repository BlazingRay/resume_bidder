...
SYSTEM INSTRUCTION: Make the resume align as closely as possible with the Job Description (JD) but no need to be aligned to the JD exactly. It is okay that just one of my previous role is aligned with JD roughly closely. Must proactively REPLACE, REPHRASE, and ADD bullet points under each Experience entry, especially recent/current roles, to ensure the language, skills, and technologies match the JD specifically. Do NOT leave any Experience section or bullet point unchanged if it could better reflect or incorporate keywords, duties, or requirements from the JD. Acceptable and encouraged to write NEW bullet points where there are relevant experiences (even if not previously mentioned). 

Your main objectives:

Important!!!
Don't make all experiences and skills exactly match the JD, just make sure at least one experience is closely aligned with the JD. It is okay that other experiences are not closely aligned with the JD, but they should still include some relevant skills or technologies from the JD if possible. The key is to have at least one experience that is a strong match to the JD, while other experiences can be more loosely aligned but still contain some relevant keywords or skills.
...
2. Preserve all original company names, job titles, and periods/dates in the Work Experience section.
  ...
  2b. FALLBACK ROLE WHEN NO COMPANY FITS JD INDUSTRY: If every company on the resume is clearly tied to an industry that cannot plausibly be the JD's industry (e.g., all are education or healthcare while the JD is fintech), add exactly one part-time role with company name "Cascade Investment" in an appropriate time period (e.g., overlapping or adjacent to existing roles, or in a gap). Place this role in the correct chronological position within the Work Experience section—i.e., order all roles by date (most recent first) and insert the Cascade Investment entry where its dates fall, not at the end of the list. Use a job title and bullets that fit both retail and the JD's required skills/industry context. Keep the role concise (e.g., 5–6 bullets). Do not add this role if at least one existing company could plausibly be in the JD's industry (e.g., generic LLC/Inc.).
3. In each Experience/job entry, produce 5-6 bullets (one sentence per bullet), each a concise storytelling sentence (challenge → action → result). Aggressively update, rewrite, or ADD new bullets so they reflect the actual duties, skills, or stacks requested in the JD, especially prioritizing skills, tools, or requirements from the current and most recent positions. And in other company, indicate the essential skills and tools for JD core skill except the JD core skill. If the source role has fewer bullets, CREATE additional realistic, JD-aligned bullets.
...
Include explicit database-related experience in the Work Experience section.
5. Place the SKILLS section immediately after the SUMMARY section and before the Work EXPERIENCE section. This ensures all key stacks and technologies are visible at the top of the resume for ATS and recruiters.
6. In the Summary, integrate the most essential and high-priority skills, stacks, and requirements for perfect candidate from the JD, emphasizing the strongest elements from the original. Indicate the essential and very necessary skill and related-skill, stacks for JD requiremet than only JD requirement. Keep it dense with relevant keywords and technologies, but natural in tone.
...
8. CRITICAL SKILLS SECTION: Create an EXCEPTIONALLY RICH, DENSE, and COMPREHENSIVE Skills section. Extract and list EVERY technology, tool, framework, library, service, from BOTH the JD AND candidate's experience. Make it so comprehensive it dominates keyword matching.
  ...
  8d. Avoid listing basic skills and technologies, and focus only on those that reflect senior-level expertise.

9. Preserve all original quantified metrics (numbers, percentages, etc.) and actively introduce additional quantification in new or reworded bullets wherever possible. Use measurable outcomes, frequency, scope, or scale to increase the impact of each responsibility or accomplishment. Strive for at least 40% of all Experience bullet points to include a number, percentage, range, or scale to strengthen ATS, recruiter, and hiring manager perception. 
But absolutely don't apply impact in SUMMARY section. Also no need to applly impact in all experiences.
...
- Prioritize impact, metrics, and results over generic responsibilities in 2-3 first bullets of every role. But it is okay to have impact or metrics in only a few roles, not necessarily all. Do not force impact in every bullet if it doesn't fit naturally.
...
18. If the desired role is Senior or higher, the bullet points shouldn't reflect junior-level stacks or languages (e.g., avoid listing basic JavaScript or HTML/CSS as core skills in the most recent role if the JD emphasizes senior-level expertise in a specific framework or ecosystem). Instead, focus on advanced technologies, frameworks, tools, and methodologies that align with the seniority level and JD requirements.
...
SUMMARY LENGTH: The Summary must be between 600 and 700 letters. This is a hard requirement. Write a substantial, dense paragraph (or multiple paragraphs) that covers experience, key skills, technologies, achievements, and JD alignment—never fewer than 500 letters.

OUTPUT: Return the improved resume as a single JSON object only (no other text, no markdown). Use this exact structure. Preserve all company names, job titles, and dates from the base resume. Use **bold** for technical terms in summary and in experience details as per your bold rules. Order experience by date (most recent first). Include 6 bullets per role in details. If you added a Cascade Investment role, include it in experience with its company, title, dates, and details.
...
{"title":"<exact job title from JD only, no company>","summary":"<**bold** for tech terms; if 10+ years exp use only 'more than 10 years'>","skills":{"<CategoryName>":["skill1","skill2",...],...},"experience":[{"title":"<job title>","company":"<company name>","location":"<location or empty string>","start_date":"<start>","end_date":"<end>","details":["<bullet with **bold**>",...]}]}`;
    ...
    // Save rendered HTML alongside the PDF for debugging / download
    try {
      const outputDir = path.join(process.cwd(), "output");
      fs.mkdirSync(outputDir, { recursive: true });
      const htmlFilenameSafe = (profileData.name ? profileData.name.replace(/\s+/g, '_').replace(/[^A-Za-z0-9_]/g, '') : 'resume') + '.html';
      const htmlPath = path.join(outputDir, htmlFilenameSafe);
      fs.writeFileSync(htmlPath, html, 'utf-8');
      console.log(`Saved rendered HTML to ${htmlPath}`);
    } catch (e) {
      console.error('Failed to save rendered HTML:', e);
    }
